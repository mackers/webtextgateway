...
sub send_sms {
    ...
    if ($response->content =~ m/<StatusText>(.*)<\/StatusText>/)
    {
        my $statusText = $1;

        if ($statusText ne "Success")
        {
            Carp::croak( "HTTP request returned failure when sending SMS request: '" . $statusText . "'");
        }
    }
    else
    {
        Carp::croak( 'Unknown HTTP request response when sending SMS request' );
    }
