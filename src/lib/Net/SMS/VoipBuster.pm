...
sub new {
    ...
    croak("Please insert a password.") unless ($password);
    croak("Please insert a subsite.") unless ($subsite);

    my $self     = bless { 'user' => $user, 'pass' => $password, 'subsite' => $subsite }, $class;
...
sub send {
    ...
    #my $send_url = $ua->get("https://myaccount." . $self->{'subsite'} . "/clx/sendsms.php?username=$self->{'user'}&password=$self->{'pass'}&from=$from&to=$to&text=$msg");
    my $send_url = $ua->get("https://www." . $self->{'subsite'} . "/myaccount/sendsms.php?username=$self->{'user'}&password=$self->{'pass'}&from=$from&to=$to&text=$msg");
